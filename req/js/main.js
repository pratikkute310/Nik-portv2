...
(function () {
  ...
  /* ── Hero canvas — the real sky over Pune, live ──────────────
   * A south-facing panorama of the night sky at 18.5204°N,
   * 73.8567°E (the coordinates on the id card), computed from
   * the actual date and time and refreshed every 30s. Bright-star
   * catalog + constellation lines; N/E/S/W compass on the
   * horizon; a rare meteor. Theme-aware, high-visibility starfield. */
  ...
  if (canvas && canvas.getContext) {
    ...
    var ox = 0, oy = 0;
    var RAD = Math.PI / 180, LAT = 18.5204 * RAD, LON = 73.8567;
    ...
    function palette() {
      return document.documentElement.getAttribute('data-theme') === 'light'
        ? { star: '13,18,16', acc: '13,122,78', meteor: '90,70,40', dim: 0.95 }
        : { star: '244,246,248', acc: '62,207,142', meteor: '255,236,210', dim: 1.08 };
    }
    var PAL = palette();
    ...
    function build() {
      ...
      var n = Math.min(160, Math.round((W * H) / 8500));
      for (var i = 0; i < n; i++) {
        var deep = Math.random() < 0.55;
        dust.push({
          x: Math.random() * W, y: Math.random() * H,
          r: 0.4 + Math.pow(Math.random(), 2.2) * (deep ? 0.65 : 1.1),
          a: 0.12 + Math.random() * (deep ? 0.22 : 0.32),
    ...
    /* Equatorial → horizontal for Pune, now; south-centred
    ...
    function spawnMeteor() {
      ...
      var speed = 3.4 + Math.random() * 2.1;
      meteors.push({
        x: W * (0.18 + Math.random() * 0.52),
        y: H * (0.03 + Math.random() * 0.16),
        vx: (ltr ? 1 : -1) * speed,
        vy: speed * (0.38 + Math.random() * 0.22),
        life: 0,
        max: 52 + ((Math.random() * 22) | 0)
      });
    }
    ...
    function draw(animate) {
      ...
      for (i = 0; i < CONS.length; i++) {
        ...
        ctx.strokeStyle = 'rgba(' + PAL.star + ',' + (0.16 * dim).toFixed(3) + ')';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ...
        if (upN && upN >= total * 0.6) {
          ...
          ctx.fillStyle = 'rgba(' + PAL.star + ',' + (0.34 * dim).toFixed(3) + ')';
      ...
      for (i = 0; i < sky.length; i++) {
        ...
        var r = Math.max(0.7, Math.min(2.85, 2.35 - st.m * 0.42));
        var a2 = Math.max(0.36, Math.min(0.96, 0.86 - st.m * 0.15));
        ...
        if (st.m < 0.2) {
          ctx.fillStyle = 'rgba(' + PAL.acc + ',' + (0.09 * dim).toFixed(3) + ')';
      ...
      for (di = 0; di < 4; di++) {
        ...
        for (dj = -1; dj <= 1; dj++) {
          ...
          ctx.strokeStyle = 'rgba(' + PAL.acc + ',' + (0.52 * dim).toFixed(3) + ')';
          ctx.lineWidth = 1;
          ctx.beginPath(); ctx.moveTo(xx, H - 30); ctx.lineTo(xx, H - 23); ctx.stroke();
          ctx.fillStyle = 'rgba(' + PAL.acc + ',' + (0.58 * dim).toFixed(3) + ')';
          ctx.fillText(DIRS[di][0], xx, H - 11);
        }
      }

      /* shooting star — rare, thin, warm-white streak */
      for (i = meteors.length - 1; i >= 0; i--) {
        ...
        ctx.lineCap = 'round';
        var glow = ctx.createLinearGradient(m.x, m.y, tailX, tailY);
        glow.addColorStop(0, 'rgba(' + PAL.meteor + ',' + (0.16 * fade).toFixed(3) + ')');
        glow.addColorStop(1, 'rgba(' + PAL.meteor + ',0)');
        ctx.strokeStyle = glow;
        ctx.lineWidth = 3.2;
        ctx.beginPath(); ctx.moveTo(m.x, m.y); ctx.lineTo(tailX, tailY); ctx.stroke();
        var core = ctx.createLinearGradient(m.x, m.y, tailX, tailY);
        core.addColorStop(0, 'rgba(' + PAL.star + ',' + (0.72 * fade).toFixed(3) + ')');
        core.addColorStop(0.45, 'rgba(' + PAL.meteor + ',' + (0.28 * fade).toFixed(3) + ')');
        core.addColorStop(1, 'rgba(' + PAL.meteor + ',0)');
        ctx.strokeStyle = core;
        ctx.lineWidth = 1.05;
        ctx.beginPath(); ctx.moveTo(m.x, m.y); ctx.lineTo(tailX, tailY); ctx.stroke();
        ctx.fillStyle = 'rgba(' + PAL.star + ',' + (0.8 * fade).toFixed(3) + ')';
        ctx.beginPath(); ctx.arc(m.x, m.y, 1.15, 0, 6.2832); ctx.fill();
      }
    }
    ...
    function loop() {
      ...
      if (frameN % 520 === 0 && Math.random() < 0.32 && meteors.length < 1) spawnMeteor();
      draw(true);
